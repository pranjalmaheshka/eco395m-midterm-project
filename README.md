...
Our code will be executed in a Python environment containing packages specified in requirements.txt. You can install them with pip install -r requirements.txt.  The /code/ folder has main.py to run the entire project. You will run this file to get the data from the university webpages. Other files: common.py requests takes a university webpage URL and returns a BeautifulSoup() instance representing the HTML of the page. scrape_pages.py scrapes professor names and titles from individual university webpages, strings.py sorts professor titles down to Assistant Professor, Associate Professor, and Professor, search_keys.py has search keys for different universities for scraping h-indexes from Google Scholar. After we run main.py, you should get a scores.csv in /artifacts/. pandas1.py analyzes the final dataframe and produces summary statistics using pandas and matplotlib. We also have graphs from our analysis stored in the /analysis/ folder.  
